defmodule Protobuf.Protoc.Generator.ExtensionTest do
  ...
  test "generate/3 generates extensions" do
    ...
    desc = Google.Protobuf.FileDescriptorProto.new(extension: [
      %{extendee: ".ext.Foo1", name: "foo", number: 1047, label: :LABEL_OPTIONAL, type: :TYPE_MESSAGE, type_name: ".ext.Options"},
      %{extendee: ".ext.Foo1", name: "foo2", number: 1049, label: :LABEL_REPEATED, type: :TYPE_UINT32},
      %{extendee: ".ext.Foo2", name: "bar", number: 1047, label: :LABEL_OPTIONAL, type: :TYPE_STRING}
    ])
  ...
  test "generate/3 generates nested extensions" do
    ...
    nested = [
      {["Parent"], [
        Google.Protobuf.FieldDescriptorProto.new(
          extendee: ".ext.Foo1", label: :LABEL_OPTIONAL, name: "foo", number: 1048, type: :TYPE_ENUM, type_name: ".ext.EnumFoo"
        )]
      }
    ]
    ...
    assert msg =~ "defmodule Ext.PbExtension do\n"
    assert msg =~ ~s(extend Ext.Foo1, :"Parent.foo", 1048, optional: true, type: Ext.EnumFoo, enum: true\n)
  end
end
