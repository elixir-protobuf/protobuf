...
defmodule Protobuf.DecoderTest do
  ...
  test "decodes one simple field" do
    struct = Decoder.decode(<<8, 42>>, TestMsg.Foo, true)
  ...
  test "decodes full fields" do
    ...
    struct = Decoder.decode(bin, TestMsg.Foo, true)
  ...
  test "raises for wrong wire type" do
    assert_raise(Protobuf.DecodeError, ~r{wrong wire_type for a: got 1, want 0}, fn ->
      Decoder.decode(<<9, 42, 0, 0, 0, 0, 0, 0, 0>>, TestMsg.Foo, true)
    end)
  end

  test "skips unknown varint fields" do
    struct = Decoder.decode(<<8, 42, 32, 100, 45, 0, 0, 247, 66>>, TestMsg.Foo, true)
    assert struct == TestMsg.Foo.new(a: 42, d: 123.5)
  end

  test "skips unknown string fields" do
    struct = Decoder.decode(<<8, 42, 45, 0, 0, 247, 66>>, TestMsg.Foo, true)
    assert struct == TestMsg.Foo.new(a: 42, d: 123.5)
  end

  test "decodes embedded message" do
    struct = Decoder.decode(<<8, 42, 50, 7, 8, 12, 18, 3, 97, 98, 99, 56, 13>>, TestMsg.Foo, true)
  ...
  test "decodes repeated varint fields" do
    struct = Decoder.decode(<<64, 12, 8, 123, 64, 13, 64, 14>>, TestMsg.Foo, true)
  ...
  test "decodes packed fields" do
    struct = Decoder.decode(<<82, 2, 12, 13>>, TestMsg.Foo, true)
    assert struct == TestMsg.Foo.new(i: [12, 13])
  end

  test "concat multiple packed fields" do
    struct = Decoder.decode(<<82, 2, 12, 13, 82, 2, 14, 15>>, TestMsg.Foo, true)
    assert struct == TestMsg.Foo.new(i: [12, 13, 14, 15])
  end

  test "decodes enum type" do
    struct = Decoder.decode(<<88, 2>>, TestMsg.Foo, true)
    assert struct == TestMsg.Foo.new(j: 2)
  end

  test "decodes unknown enum type" do
    struct = Decoder.decode(<<88, 3>>, TestMsg.Foo, true)
    assert struct == TestMsg.Foo.new(j: 3)
  end

  test "decodes map type" do
    struct = Decoder.decode(<<106, 12, 10, 7, 102, 111, 111, 95, 107, 101, 121, 16, 213, 1>>, TestMsg.Foo, true)
    assert struct == TestMsg.Foo.new(l: %{"foo_key" => 213})
  end

  test "decodes 0 for proto2" do
    assert Decoder.decode(<<8, 0, 17, 5, 0, 0, 0, 0, 0, 0, 0>>, TestMsg.Foo2, true) == TestMsg.Foo2.new(a: 0)
  end

  test "decodes [] for proto2" do
    assert Decoder.decode(<<8, 0, 17, 5, 0, 0, 0, 0, 0, 0, 0>>, TestMsg.Foo2, true) == TestMsg.Foo2.new(a: 0, g: [])
  end

  test "decodes %{} for proto2" do
    assert Decoder.decode(<<8, 0, 17, 5, 0, 0, 0, 0, 0, 0, 0>>, TestMsg.Foo2, true) == TestMsg.Foo2.new(a: 0, l: %{})
  end

  test "decodes custom default message for proto2" do
    assert Decoder.decode(<<8, 0, 17, 0, 0, 0, 0, 0, 0, 0, 0>>, TestMsg.Foo2, true) == TestMsg.Foo2.new(a: 0, b: 0)
    assert Decoder.decode(<<8, 0>>, TestMsg.Foo2, true) == TestMsg.Foo2.new(a: 0, b: 5)
  end

  test "oneof only sets oneof fields" do
    assert Decoder.decode(<<42, 5, 111, 116, 104, 101, 114, 8, 42, 34, 3, 97, 98, 99>>, TestMsg.Oneof, true) ==
      %TestMsg.Oneof{first: {:a, 42}, second: {:d, "abc"}, other: "other"}
    assert Decoder.decode(<<42, 5, 111, 116, 104, 101, 114, 18, 3, 97, 98, 99, 24, 123>>, TestMsg.Oneof, true) ==
